...
TRAIN_STATE = False #是否是训练状态

#从文件读取数据
def ReadFile(FileInPath, TextList):
    FileNames = os.listdir(FileInPath)
    for FileName in FileNames:
        FileName = FileInPath + FileName
...
#读取测试集文件
def read_testFile(FileName,TextList):
    with open(FileName,'r',encoding = 'utf-8') as F:
        lines = F.readlines()
        for line in lines:
            line.strip()
...
def Text_To_CleanText(TextList,CleanPath):
    #WordsList = []
...
def DataClean(WordList):
    ...
    stoplist = ['very', 'ourselves', 'am', 'doesn', 'through', 'me','Me', 'against', 'up', 'just', 'her', 'ours','Ours'
                'could', 'because', 'is', 'its', 'it', 'only','Only' 'in', 'such', 'too', 'must', 'under','Under' 'their','Their'
                'if', 'to', 'my', 'himself', 'after', 'why', 'while', 'can', 'each', 'itself', 'his', 'all', 'once',
                'herself', 'more', 'our', 'the','they', 'hasn', 'on', 'ma', 'them', 'its', 'where', 'did', 'All', 'you',
                'did', 'nor', 'as', 'now', 'before', 'those', 'yours', 'from', 'who', 'was', 'm', 'been', 'will',
                'into', 'same', 'how', 'some', 'of', 'out', 'with', 's', 'being', 't', 'might', 'she', 'She', 'again', 'be',
                'by', 'sha', 'have', 'yourselves', 'need', 'and', 'are', 'o', 'these', 'further', 'most', 'yourself',
                'having', 'are', 'here', 'he', 'were', 'but', 'this', 'myself', 'own', 'we','We', 'so', 'i','I' 'does', 'both',
                'when', 'between', 'd', 'had', 'the', 'y', 'has', 'down', 'off', 'than', 'haven', 'whom', 'would',
                'should', 've', 'over', 'themselves', 'few', 'then', 'hadn', 'what', 'until', 'won', 'no', 'about',
                'any', 'that', 'for', 'should', 'Do','does', 'do', 'there', 'doing', 'an', 'or', 'ain', 'hers', 'wasn',
                'were', 'above', 'a','A', 'at', 'At','your', 'theirs', 'below', 'other', 'not', 're', 'him', 'during', 'which','br']
    punclist = [s for s in string.punctuation]
    new_WorldList = []
    for w in WordList:
        '''(w not in stoplist) and (not w.isdigit()) and'''
...
def build_vocab(WordsList):
    ...
    for Words in WordsList:
        for Word in Words:
            ...
            f.write(Word + ' ' + str(i) + '\n')
            i = i + 1
    f_glove = open('embedding/glove.6B.300d.txt','r',encoding = 'utf-8')
...
def build_Wordembedding(word_index):
    EMBEDDING_FILE = "./embedding/glove.6B.300d.txt"
...
def process_input(WordsList, words_vocab,truncate=False):
    ...
    new_WordsList = [[vocab[word] for word in Words] for Words in WordsList]
        ...
        #new_WordsList = [line[:TRAIN_MAX_SENTENCE_LEN] for line in new_WordsList]
...
pos_dev = []
neg_dev = []
ReadFile("D:/ProgramData/python/Task/One/data/train/neg/",neg_train)
ReadFile("D:/ProgramData/python/Task/One/data/train/pos/",pos_train)
ReadFile("D:/ProgramData/python/Task/One/data/dev/neg/",neg_dev)
ReadFile("D:/ProgramData/python/Task/One/data/dev/pos/",pos_dev)
#neg_train_WordsList = TextList_To_WordsList(neg_train)
#pos_train_WordsList = TextList_To_WordsList(pos_train)
Text_To_CleanText(neg_train,'./data/train/neg_clean.txt')
Text_To_CleanText(pos_train,'./data/train/pos_clean.txt')
Text_To_CleanText(neg_dev,'./data/dev/neg_clean.txt')
Text_To_CleanText(pos_dev,'./data/dev/pos_clean.txt')'''

#读取测试集数据&预处理
#testList = []
#read_testFile('data/test.txt',testList)
#Text_To_CleanText(testList,'data/test_clean.txt')
...
if __name__ == "__main__":
    if TRAIN_STATE:
            ...
            for batched_X, batched_Y in train_dataloader:
                ...
                if global_step % 500 == 0:
                    ...
                    if acc > best_acc:
                        ...
                        torch.save(cnn_Model, 'best_model.bin')
        ...
        print("Test start...")
        cnn_Model = torch.load('best_model.bin')
        cnn_Model.eval()
        predict_Y = []  # 所有的输出
        X_test_result = []
        real_Y = []  # 所有的标签
